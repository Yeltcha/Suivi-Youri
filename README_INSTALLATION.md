...
La version 2.0.0 recentre INNERSET sur un carnet d’entraînement personnel : programmes libres, saisie série par série et progression fondée sur des passages réellement comparables. Elle conserve toutes les données déjà enregistrées et ne demande aucune modification de la table Supabase.
...
5. Ouvre **Profil et sauvegarde** depuis le bouton en haut à droite, puis crée ton compte ou connecte-toi pour activer la synchronisation entre appareils.
...
- Utilisez régulièrement **Profil et sauvegarde → Exporter une sauvegarde** pour créer un fichier JSON restaurable.
...
La bibliothèque, les programmes libres, les structures de séries, les cibles exactes ou en fourchette, les réglages RIR et W, l’état de validation, les ressentis et les séances historiques utilisent la ligne `training_state` déjà créée dans Supabase. La version 2.0.0 conserve le schéma JSON interne en version 10 : aucune nouvelle table ni nouvelle requête SQL n’est nécessaire.
